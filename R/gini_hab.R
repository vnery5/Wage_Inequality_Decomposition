## SCRIPT QUE CALCULA O GINI PARA RENDA DO TRABALHO E RENDA HABITUAL DOMICILIAR 
...
# df11$VD4019 <- df11$VD4019*df11$Habitual
# 
...
# df2$VD4019 <- df2$VD4019*df2$Habitual
# 
...
# df3$VD4019 <- df3$VD4019*df3$Habitual
# 
# df <- rbind(df11,df2)
# df <- rbind(df,df3)
# 
...
# 
# ## vendo a renda média
# svyby(~VD4019, by = ~Ano, df, svymean, na.rm  =  TRUE)
# svymean(~VD4019, df11,na.rm  =  TRUE, ci=TRUE,vartype="ci")
# svymean(~VD4019, df2,na.rm  =  TRUE, ci=TRUE,vartype="ci")
# svymean(~VD4019, df3,na.rm  =  TRUE, ci=TRUE,vartype="ci")
# svyby(~VD4019, by = ~Ano, df4, svymean, na.rm  =  TRUE)
# 
# df11 <- NULL
# df2 <- NULL
# df3 <- NULL
...
table(df.tot$Ano)
table(df.tot$Ano[!is.na(df.tot$VD4019)])
table(df.tot$Ano[!is.na(df.tot$VD4019)])/table(df.tot$Ano)
...
table(df.tot$Ano[df.tot$V2009 >= 15 & df.tot$V2009 < 65 & !is.na(df.tot$VD4019)])
table(df.tot$Ano[df.tot$V2009 >= 15 & df.tot$V2009 < 65 & !is.na(df.tot$VD4019)])/table(df.tot$Ano[df.tot$V2009 >= 15 & df.tot$V2009 < 65])
...
df.tot$VD4019 <- df.tot$VD4019*df.tot$Habitual

#### renda domiciliar (de todos os trabalhos)
df.tot$renda.trab.hab.dom <- ave(df.tot$VD4019, df.tot$iddom, df.tot$data, FUN = function(x) sum(x, na.rm = TRUE))
df.tot$renda.hab.dom.pc <- df.tot$renda.trab.hab.dom/df.tot$VD2003

## renda por hora (VD4031 e VD4035 são horas por semana)
df.tot$sal.hab.hora <- df.tot$VD4019/(df.tot$VD4031*365/(7*12))

# por permitir a logaritmização, opta-se pela renda habitual de todos os trabalhos
# (já deflacionada pelo deflator CO1)
names(df.tot)[names(df.tot) == 'VD4019'] <- 'sal.hab'
...
uteis <- c('Ano','UPA','Estrato','posest','idade','V1027','V1028','V1029','sexo','cor','cor2','sal.hab','renda.hab.dom.pc')
df.tot <- df.tot[,uteis]
...
df.tot1 <- df.tot1[df.tot1$idade >= 15 & df.tot1$idade < 64 & !is.na(df.tot1$sal.hab) & !is.na(df.tot1$cor), ]
...
media.sal <- svyby(~sal.hab, by = ~Ano, 
                   subset(df.tot, idade >= 15 & idade < 65 & !is.na(sal.hab) & !is.na(cor)),
...
mediana.sal <- svyby(~sal.hab, by = ~Ano,
                     subset(df.tot, idade >= 15 & idade < 65 & !is.na(sal.hab) & !is.na(cor)),
...
################## Rendas Médias e Medianas Habituais Domiciliares Per Capita ##############
Sys.time()
media.pc <- svyby(~renda.hab.dom.pc, by = ~Ano, df.tot,
                   svymean, na.rm = TRUE, ci = TRUE, vartype="ci")
Sys.time()
...
Sys.time()
mediana.pc <- svyby(~renda.hab.dom.pc, by = ~Ano, df.tot,
...
Sys.time()
gini.trab <- svyby(~sal.hab, by = ~Ano, 
                   subset(df.tot, idade >= 15 & idade < 65 & !is.na(sal.hab) & !is.na(cor)),
...
gini.tot <-svyby(~renda.hab.dom.pc, by = ~Ano, 
                 df.tot, 
...
grafico.renda
ggsave("renda_media_mediana.pdf", dpi = 1200)
...
grafico.renda.pc <- renda.pc %>% 
  ...
  geom_ribbon(aes(ymin = ci.l, ymax = ci.u, fill = variable), alpha = 0.3, show.legend = F) +
  scale_colour_manual(name = "",
                      labels = c("RTDPC Média",' RTDPC Mediana'),
                      values=c("black","gray40")) + 
  scale_fill_manual(values=c("black","gray40")) + 
...
## calculando a média geral de sal.hab pra comparar com os resultados do stata!
# média: R$ 2.443,9; erro padrão: 9,4833
# media.renda.total <- svymean(~sal.hab, df.tot, na.rm = T, ci = T)
# media.renda.total

## coluna dos grupos
df.tot$variables$cor2.sexo <- paste(df.tot$variables$cor2, df.tot$variables$sexo, sep = ".")

## fazendo a renda média utilizando o svy (e vendo quanto tempo demora)
t1 <- Sys.time()
media.sal.desag <- svyby(~sal.hab, by = ~Ano + cor2.sexo, 
                         subset(df.tot, idade >= 15 & idade < 65 & !is.na(sal.hab) & !is.na(cor)), 
                         svymean, na.rm = T, ci = T, vartype = "ci")
T1 <- Sys.time() - t1
T1

## formatando o dataframe
media.sal.desag$categ <- media.sal.desag$cor2.sexo
media.sal.desag <- media.sal.desag[,c('Ano','categ','sal.hab','ci_l','ci_u')]
media.sal.desag

## gráfico com título
grafico.desag <- media.sal.desag %>% 
  ggplot(aes(x=Ano)) +
  geom_point(
    data = subset(media.sal.desag, Ano %in% c(2012,2014,2016,2018,2020,2021)), 
    aes(y = sal.hab, colour = categ), 
    shape = 18, size = 2
  ) +
  geom_text(
    data = subset(media.sal.desag, Ano %in% c(2012,2014,2016,2018,2020,2021)), 
    aes(y = sal.hab, label = format(round(sal.hab), big.mark = ".", decimal.mark = ",")), 
    nudge_y = 85, size = 3
  ) +
  geom_line(aes(y = sal.hab, colour = categ), size = 1) +
  geom_ribbon(aes(ymin = ci_l, ymax = ci_u, fill = categ), alpha = 0.3, show.legend = F) +
  scale_colour_hue(
     name = "",
     labels = c("Homem Branco",'Mulher Branca','Homem PPI','Mulher PPI')
   ) + 
  labs(x = 'Ano', y = 'Renda (R$ 2021)', 
       title = 'Evolução da Renda Média do Trabalho por Grupo por Ano',
       caption = 'Fonte: Microdados da PNADC.\nElaboração a partir do pacote Survey.') +
  scale_x_continuous(breaks = seq(2012,2021, by = 1)) +
  scale_y_continuous(breaks = seq(1000,4200, by = 500), labels=function(x) format(x, big.mark = ".", decimal.mark = ",", scientific = FALSE)) +
  theme_bw() + 
  theme(
    legend.position = 'top',
    legend.direction = 'horizontal',
...
grafico.desag
ggsave("renda_sexo_cor.pdf",dpi=300)

## gráfico com caption
grafico.desag.capt <- media.sal.desag %>% 
  ggplot(aes(x=Ano)) +
  geom_point(
    data = subset(media.sal.desag, Ano %in% c(2012,2014,2016,2018,2020,2021)), 
    aes(y = sal.hab, colour = categ), 
...
grafico.desag.capt
ggsave("renda_sexo_cor_caption.pdf",dpi=300)
