...
svy, subpop(ativo if vinte == 1): reg lsal_efet $x_setor
eststo reg2020_efet

svy, subpop(ativo if vinteum == 1): reg lsal_efet $x_setor
eststo reg2021
...
* Feminino: redução da discriminação ao longo do tempo (o que coincide com a maior participação feminina no mercado de trabalho); fenomêno se inverte 2m 2921
* PPI: redução da discriminação entre 2012 e 2015, mas aumento entre 2015-2020/21 para patamares semelhantes ao de 2012
* Rural: gap estável; quando se considera o salário por hora, há uma diminuição do gap, uma vez que pessoas em ambientes rurais trabalham menos horas
...
* Região (frente ao CO, principalmente DF): N aumentou o gap; NE diminuiu entre 2012 e 2015, mas aumentou novamente com a crise. S fez esse gap quase zerar e SE diminuiu bastante.
* Setor (frente a serviços financeiros): Adm. Pública aumentou seu prêmio e agro diminuiu; gap do comércio aumentou com a pandemia. Chama atenção o setor de construção, que havia aumentado sua vantagem em 2015 e, com o advento dos escandâlos de corrupção descobertos pela Lava-Jato, viu seu prêmio salarial cair novamente. Vale mencionar também a melhora dos serviços domésticos (quando se considera a renda por hora), apesar da perda parcial do avanço com a pandemia.
cls
esttab reg2012 reg2015 reg2020 reg2020_efet reg2021, b(4) se(4) r2(3) ar2(3)  ///
title("\label{tab1}Estimações (com desenho amostral) por ano") ///
nonumbers mtitles("2012" "2015" "2020" "2020_efet" "2021") ///
refcat("educ" "Educação" "exper" "\rule{0pt}Experiência" "2.regiao" "\rule{0pt}Região" "1.setor" "\rule{0pt}{4ex}Setor", nolabel) ///
...
*** 2020 e 2021 ***
** Média
quietly oaxaca_rif lsal_efet $xlist_oaxaca if ativo == 1 & (Ano == 2021 | Ano == 2020) [pw = V1028], rif(mean) by(vinte) wgt(0) robust
eststo media_2021
...
foreach i in 1 10 50 90 99 {
	display `i'
	quietly oaxaca_rif lsal_efet $xlist_oaxaca if ativo == 1 & (Ano == 2021 | Ano == 2020) [pw = V1028], rif(q(`i')) by(vinte) wgt(0) robust relax
	eststo p`i'_2021
}
...
* 2012-2020, ESTRUTURAL: A redução no prêmio educacional (ainda que insignificante a 10%) - principalmente no 1º período - puxou a renda média para baixo, o que foi parcialmente contrabalanceado pela menor discriminação feminina, menos desigualdade interregional e maior prêmio de formalidade. Além disso, a constante também teve um fator importantissimo na redução da renda, indicando o fim do ciclo dos salários mínimos ou a desvalorização de habilidades não-observáveis, o que pode ser um sinal da precarização do mercado de trabalho em 2015-2020.

* 2020-2021, COMPOSIÇÃO: Em linha com o IQT, a FT ficou mais escolarizada (76,6%% do efeito total) e experiente (29,9%) (principalmente no topo da distribuição), haja vista a perda de postos mais precários. Educação e experiência são, juntos, responsáveis por 106,5% do efeito composição. Há uma FT menos diversa (já que mulheres, PPIs e sua interação foram os mais afetados e possuiam menores rendimentos). Dado que a agricultura foi o setor menos afetado, houve uma maior importância relativa destes postos de trabalho na distribuição; como eles tem renda média menor, há o sinal negativo. Houve uma perda de formalização e um movimento para setores menores produtivos, ainda que insignificante.
*2020-2021, ESTRUTURAL: Houve uma GIGANTESCA redução do prêmio educacional (insignificante) e de experiência, fenômeno que ficou concentrado na base da distribuição: indivíduos mais escolarizados foram forçados a, por necessidade, ocuparem postos mais precários e menos bem-remunerados, diminuindo sua renda e piorando a produtividade de curto e de longo prazo do país (histerese). Na contrapartida, há um grande aumento do prêmio de outros fatores não-observados, os quais não são inteiramente claros. Além disso, uma redução no retorno experiência causou uma queda na renda média, provavelmente através do mesmo mecanismo da educação ou em virtude da adoção de novas tecnologias que favorecem os mais jovens ("age-biased technical change hypothesis, BEHAGHEL e GREENAN, 2010). O sinal de região mostra que os efeitos da crise prejudicaram mais as regiões mais pobres. Houve uma valorização do prêmio para os trabalhadores rurais.
...
* 2012-2015, ESTRUTURAL: A estrutura do mercado de trabalho mais que compensou o efeito composicão, sendo o principal driver a queda no prêmio educacional e, principalmente, da experiência, em linha com os achados de FERREIRA et al (2021). Isso se deve a revolução computacional/técnica que beneficia os mais novos, que tradicionalmente estão na parte de baixo da pirâmide salarial. Ressalta-se que houve um aumento da desigualdade inter-regional no período, sendo os demais efeitos insignificantes ao nível de 10%.
...
forvalues i = 1(1)99 {
	...
	eststo: quietly oaxaca_rif $y $xlist_oaxaca if ativo == 1 & (Ano == 2012 | Ano == 2015) [pw = V1028], rif(q(`i')) by(doze) wgt(0) nose relax
}
set dp period
...
forvalues i = 1(1)99 {
	display "Calculando para o " `i' "º percentil (" `i' "%)..."
	eststo: quietly oaxaca_rif $y $xlist_oaxaca if ativo == 1 & (Ano == 2015 | Ano == 2020) [pw = V1028], rif(q(`i')) by(quinze) wgt(0) nose relax
}
set dp period
...
forvalues i = 1(1)99 {
	display "Calculando para o " `i' "º percentil (" `i' "%)..."
	eststo: quietly oaxaca_rif $y $xlist_oaxaca if ativo == 1 & (Ano == 2012 | Ano == 2020) [pw = V1028], rif(q(`i')) by(doze) wgt(0) nose relax
}
set dp period
...
forvalues i = 1(1)99 {
	display "Calculando para o " `i' "º percentil (" `i' "%)..."
	eststo: quietly oaxaca_rif lsal_efet $xlist_oaxaca if ativo == 1 & (Ano == 2020 | Ano == 2021) [pw = V1028], rif(q(`i')) by(vinte) wgt(0) nose relax
}
set dp period
